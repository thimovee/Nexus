...
export function DataTable<TData extends Order, TValue>({ columns, data }: DataTableProps<TData, TValue>) {
    ...
    return (
        <div className="pb-10">
            <div className="flex mb-6 w-full ">
                ...
                {Object.values(OrderStatus).sort().map(status => (
                    <div key={status} className={cn("hover:text-primary hover:cursor-pointer transition duration-150 flex gap-2 items-center font-semibold text-sm  px-5 py-2 border-b-2 text-primary/50 border-border", table.getColumn("status")?.getFilterValue() === status.toString() && "text-cta border-cta hover:text-cta")} onClick={() => table.getColumn("status")?.setFilterValue(status.toString())}>
                        {/* make it so the statuses like this: COMPLETED... etc are Displayed like this: Completed */}
                        <span className="lowercase first-letter:uppercase">{status}</span>
                        <div className="bg-border h-[18px] text-xs w-[18px] p-1 rounded-md flex items-center justify-center">
                            {data.filter((order: Order) => order.status === status).length}
                        </div>
                    </div>
                ))}
                ...
                <div className="flex-1 border-b-2 border-border flex justify-end">
                    <div className="flex gap-2 items-center mb-4">
                        <ExportDataButton background data={data} name="orders" />
                    </div>
                </div>
            </div>
